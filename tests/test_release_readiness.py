...
def write_minimal_package_zip(
  ...
  files: dict[str, bytes | str] = {
    ...
    "OPEN-3D-MAC.command": "#!/bin/sh\n",
    "OPEN-3D-WINDOWS.bat": "@echo off\n",
    "index.html": "<!doctype html>\n",
    "favicon.svg": "<svg></svg>\n",
    "dzi/regierungsviertel/pedestrian_map.png": b"png",
    "dzi/regierungsviertel/startup-map.jpg": b"jpg",
  ...
  if "package-manifest.json" not in files:
    asset_paths = {
      "pedestrian_map": "dzi/regierungsviertel/pedestrian_map.png",
      "startup_backdrop": "dzi/regierungsviertel/startup-map.jpg",
...
def write_minimal_static_tarball(
  ...
  files: dict[str, bytes | str] = {
    ...
    "dzi/regierungsviertel/tiergartentunnel.json": b"{}",
...
def test_viewer_binary_size_failures_rejects_oversized_preview(tmp_path: Path) -> None:
  ...
  preview = tmp_path / "pedestrian_map.png"
  ...
  assert len(failures) == 1
  assert "pedestrian_map.png" in failures[0]
...
def test_zip_package_failures_rejects_stale_launcher(tmp_path: Path) -> None:
  ...
  failures = release_readiness.zip_package_failures(tmp_path)
  assert any("file-unsafe content" in failure for failure in failures)
      ...
      "package-manifest.json": json.dumps(
        {
          ...
          "assets": {
            "pedestrian_map": {
              "path": "dzi/regierungsviertel/pedestrian_map.png",
              "bytes": 3,
              "sha256": "0" * 64,
            }
          },
...
@pytest.mark.parametrize(
  ...
  [
    ...
    "dzi/regierungsviertel/preview.html",
  ],
)
def test_release_archives_reject_retired_flat_map_outputs(
  tmp_path: Path, relative: str
) -> None:
  checker = load_script_module(
    "readiness_no_flat_outputs", "scripts/check_release_readiness.py"
  )
  write_minimal_package_zip(tmp_path, checker, {relative: b"retired"})
  assert any(
    "Retired flat-map package ZIP asset" in failure
    for failure in checker.zip_package_failures(tmp_path)
  )
  write_minimal_static_tarball(tmp_path, checker, {relative: b"retired"})
  assert any(
    "Retired flat-map static archive asset" in failure
    for failure in checker.static_tarball_failures(tmp_path)
  )
...
def test_release_archives_require_retained_navigation_images(
  tmp_path: Path, relative: str
) -> None:
  checker = load_script_module(
    "readiness_navigation_images", "scripts/check_release_readiness.py"
  )
  write_minimal_package_zip(tmp_path, checker, {relative: None})
  assert any(
    "Missing package ZIP entry" in failure and relative in failure
    for failure in checker.zip_package_failures(tmp_path)
  )
  write_minimal_static_tarball(tmp_path, checker, {relative: None})
  assert any(
    "Missing static archive entry" in failure and relative in failure
    for failure in checker.static_tarball_failures(tmp_path)
  )


def test_launch_guide_rejects_unguarded_file_redirect() -> None:
  checker = load_script_module(
    "readiness_file_safe_guide", "scripts/check_release_readiness.py"
  )
  broken = VALID_START_HERE_HTML.replace('window.location.protocol === "http:"', "true")
  assert any(
    "HTTP-only redirect" in failure
    for failure in checker.package_start_here_failures(broken, "fixture")
  )
...
def test_mode_readiness_requires_live_desktop_and_mobile_controls(
  monkeypatch: pytest.MonkeyPatch,
  missing: str,
) -> None:
  checker = load_script_module(
    "readiness_live_visual_modes", "scripts/check_release_readiness.py"
  )
  app_path = ROOT / "src/app/src/App.tsx"
  original_read = Path.read_text
  app = original_read(app_path, encoding="utf-8")
  assert missing in app
  assert "toggleLightingMode" not in app
  assert checker.webgl_viewer_source_failures(ROOT) == []

  def read_with_missing_control(path: Path, *args, **kwargs) -> str:
    content = original_read(path, *args, **kwargs)
    # Removing only one of two selection bindings catches loss of either UI.
    return (
      content.replace(missing, "missing control", 1) if path == app_path else content
    )

  monkeypatch.setattr(Path, "read_text", read_with_missing_control)
  failures = checker.webgl_viewer_source_failures(ROOT)
  assert any(
    "connected visual-style controls" in failure or "desktop/mobile" in failure
    for failure in failures
  )
