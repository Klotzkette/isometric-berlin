...
def test_write_start_here_provides_file_safe_3d_help_and_http_redirect(
  tmp_path: Path,
) -> None:
  package = load_script_module("package_static_site", "scripts/package_static_site.py")
  checker = load_script_module(
    "release_start_guide", "scripts/check_release_readiness.py"
  )
  (tmp_path / "index.html").write_text(
    '<script type="module" src="./assets/index.js"></script>'
  )
  package.write_start_here(tmp_path)
  html = (tmp_path / "START-HERE.html").read_text()
  assert checker.package_start_here_failures(html, "fixture") == []
  assert "mapImage" not in html
  assert "overview.png" not in html
  assert "regierungsviertel.dzi" not in html
  assert len(html.encode()) < 8_000


def test_start_guide_does_not_require_legacy_map_assets(tmp_path: Path) -> None:
  package = load_script_module(
    "package_missing_build", "scripts/package_static_site.py"
  )
  with pytest.raises(SystemExit, match="Missing packaged 3D viewer"):
    package.write_start_here(tmp_path)
  (tmp_path / "index.html").write_text("3D app")
  package.write_start_here(tmp_path)
  assert (tmp_path / "START-HERE.html").is_file()
...
def test_package_readme_mentions_version_and_port_fallback(tmp_path: Path) -> None:
  ...
  assert "2D-Notansicht" not in readme
  assert "START-HERE.html is a file-safe launch guide" in readme
...
def test_write_package_manifest_records_version_hashes_and_attribution(
  ...
  files = {
    ...
    "dzi/regierungsviertel/pedestrian_map.png": b"minimap",
    "dzi/regierungsviertel/startup-map.jpg": b"backdrop",
  ...
  assert manifest["start_page_mode"] == "3d-launch-guide"
  assert manifest["full_3d_start_page"] == "index.html"
  assert "preferred_image" not in manifest
  assert "dzi_descriptor" not in manifest
  ...
  assert manifest["assets"]["pedestrian_map"]["bytes"] == len(b"minimap")
  assert len(manifest["assets"]["pedestrian_map"]["sha256"]) == 64
...
def test_packaging_drops_flat_exports_and_keeps_every_3d_asset(tmp_path: Path) -> None:
  package = load_script_module(
    "package_isometric_only", "scripts/package_static_site.py"
  )
  source = tmp_path / "source"
  target = tmp_path / "package"
  retained = {
    "index.html": b"3d",
    "assets/viewer.js": b"three",
    "dzi/regierungsviertel/pedestrian_map.png": b"minimap",
    "dzi/regierungsviertel/startup-map.jpg": b"startup",
    "dzi/regierungsviertel/landmarks.json": b"landmarks",
    "dzi/regierungsviertel/tiergartentunnel.json": b"tunnel",
    "dzi/regierungsviertel/wikimedia_attribution.json": b"credits",
    "dzi/regierungsviertel/visual_reference_attribution.json": b"credits2",
    **{
      f"mesh/regierungsviertel/{name}.json": name.encode()
      for name in (
        "scene",
        "ground-context",
        "lod2-prisms",
  ...
  retired = (
    "regierungsviertel_files/0/0_0.jpg",
    "regierungsviertel_files/8/0_0.jpg",
    "overview.png",
    "overview_source.png",
    "reference_map.png",
  ...
  files = {
    **retained,
    **{f"dzi/regierungsviertel/{name}": b"retired" for name in retired},
  }
  for relative, body in files.items():
    path = source / relative
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(body)
  package.copy_static_site(source, target)
  package.ensure_public_viewer_support_copied(source, target)
  assert {
    p.relative_to(target).as_posix(): p.read_bytes()
    for p in target.rglob("*")
    if p.is_file()
  } == retained
  # ZIP and tar filter an already staged legacy file too.
  stale = target / "dzi/regierungsviertel/overview.png"
  stale.write_bytes(b"retired")
  package.zip_package(target, tmp_path / "viewer.zip")
  package.tar_static_site(target, tmp_path / "viewer.tar.gz")
  with zipfile.ZipFile(tmp_path / "viewer.zip") as archive:
    assert set(archive.namelist()) == {f"package/{name}" for name in retained}
  with tarfile.open(tmp_path / "viewer.tar.gz") as archive:
    assert set(archive.getnames()) == set(retained)


def test_repo_server_requires_real_3d_build_instead_of_flat_preview(
  tmp_path: Path,
) -> None:
  server = load_script_module("server_isometric_only", "scripts/serve_local_viewer.py")
  preview = tmp_path / "src/app/public/dzi/regierungsviertel/preview.html"
  preview.parent.mkdir(parents=True)
  preview.write_text("obsolete")
  with pytest.raises(SystemExit, match="bun install && bun run build"):
    server.viewer_directory(tmp_path)
  dist = tmp_path / "src/app/dist"
  dist.mkdir(parents=True)
  (dist / "index.html").write_text("3D app")
  assert server.viewer_directory(tmp_path) == (dist, "/")
