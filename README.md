...
unchanged. See [validation](docs/release-v1.0.43-review.md).

Version 1.0.43 hält W beim Schwenken durchgehend aktiv, auch nach dem Loslassen
einer Pfeiltaste. Tastenbelegung und Stadtdetails bleiben erhalten.
...
Version 1.0.39 verbessert das GRIPS-Theater und die kleinen Ladenhöfe am Hansaplatz: verglaster Eingang, rote Schriftzüge, erhöhter Glasgang und offene überdachte Wege, mit quellengebundenen Bauformen.
...
Naturkundemuseum, Dussmann and existing central facades, with continuous mapped
Spree-side streets and source-bound railings. Quality and navigation remain intact.
See [stability](docs/mobile-stability-v137.md), [facades](docs/urban-architecture-relief-v137.md)
and [Spree sources](docs/spree-streets-v137.md).

Version 1.0.37 verteilt den Stadtaufbau auf abbrechbare Arbeitsschritte und gibt
entfernte Geometrien auch aus der GPU-Vorbereitung frei. Schloss, Naturkundemuseum,
Dussmann und zentrale Fassaden werden genauer; kartierte Straßen und Geländer
vervollständigen die Spreeufer. Detailtreue und Steuerung bleiben erhalten.
...
Version 1.0.35 macht vorhandene Fassadenfarben und Materialien in den benannten
zentralen Vierteln deutlicher und stärkt vorhandene Fassadenlinien und Konturen.

Version 1.0.34 turns both Soviet Memorial tanks toward the street and refines
the Rosengarten's mapped flower beds, open stone pergola and benches.

Version 1.0.34 richtet beide Panzer am Sowjetischen Ehrenmal zur Straße aus und
verfeinert die kartierten Rosenbeete, die offene Steinpergola und die Bänke.

Version 1.0.33 reduces mobile source-data and ground-construction memory while
keeping geometry, detail, draw distance and navigation unchanged.

Version 1.0.33 senkt den Speicherbedarf mobiler Quelldaten und des Bodenaufbaus,
bei unveränderter Geometrie, Detailtreue, Sichtweite und Steuerung.
...
The mapped lawns, gravel median, buildings and smooth navigation are retained.

Version 1.0.30 stellt Pariser Platz und Unter den Linden bis zur Friedrichstraße
mit durchgehenden Platzflächen, geraden Fahrbahnrändern, Bordsteinen und erhöhten
Gehwegen dar. Erfasste Beete, Kiespromenade, Gebäude und flüssige Steuerung bleiben erhalten.
...
Version 1.0.23 korrigiert ausschließlich Richard Serras Berlin Junction: gebogene,
nach innen geneigte Stahlplatten, dunklere Patina und ein offener Durchgang
in allen fünf Modi.
...
The ministry gains differentiated historic and modern roofs with a solar array;
Minecraft retains all mapped Invalidenfriedhof graves.

Version 1.0.14 zeigt die Gebäude am Humboldthafen und das Wirtschaftsministerium
vom ersten Bild an mit ihren tatsächlichen Grundrissen, offenen Höfen und freiem
Wasser. Das Ministerium erhält unterscheidbare historische und moderne Dächer
mit Solaranlage; Minecraft zeigt alle erfassten Gräber des Invalidenfriedhofs.
...
refinement. The historical Potsdamer Platz traffic tower gains its pentagonal
frame, glazed cabin, clocks and slowly cycling horizontal signals.

Version 1.0.12 zeigt die gesamte Stadt schon ab der ersten bedienbaren Ansicht
räumlich, auch bei mobilen Tabwechseln und verzögerter Detailverfeinerung.
Der historische Verkehrsturm am Potsdamer Platz erhält seinen fünfeckigen
Rahmen, die Glaskabine, Uhren und langsam wechselnde horizontale Lichtsignale.

Version 1.0.11 rebuilds the BahnTower with its curved glass facade, DB sign,
stepped roof and fully illuminated night-time floors. The Musikinstrumenten-
museum receives corrected absolute roof heights, full-width rooflights and
its accurately positioned entrance pavilion. The Spreebogen bank gains
continuous paving, source-bound planting and coherent terrain transitions.

Version 1.0.11 gibt dem BahnTower seine gebogene Glasfassade, das DB-Logo,
das gestaffelte Dach und durchgehend helle Etagen bei Nacht. Beim Musik-
instrumentenmuseum stimmen Dachhöhen, Oberlichter und Eingangspavillon.
Das Spreebogenufer erhält zusammenhängende Wege, richtig platzierte
Grünflächen und korrigierte Geländeübergänge.
...
- **Kollhoff panorama colours at Potsdamer Platz.** Twenty complete source
  ...
  storeys, blue-grey glazing, planted roofs and pale southern housing.
  Tilla-Durieux lawns and tree rows have quieter local greens. Two roof-light
  surfaces join existing batches without photographs or extra draw calls.
  [Evidence and bounded cost](docs/potsdamer-panorama.md).

- **Faster, interruptible Minecraft construction.**
  ...
  The v0.72.41-to-v0.72.42 optimization preserved full/mobile output bytewise;
  the later panorama refinement intentionally updates its appearance baseline.
...
- **Less startup work, identical architectural contours.** Repeated box outlines
  ...
  Thin or degenerate parts retain the original classifier. The fixed contour
  benchmark verifies identical output bytes; mapped Tilla-Durieux lawns use
  bounded point tests without changing their footprints or terrain.
...
- **Distant source buildings retain their colours during Worker transfer.**
  ...
  explicitly uploaded. Even when an older cached Worker payload reaches the
  current viewer, the transfer boundary repairs that legacy material state
  before its first draw. Progressive loading therefore keeps a continuous city
