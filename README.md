...
> **Status:** Local v0.1.27 open-data viewer with a more architectural render pass: layered contact shadows, crisper podium/cornice lines, paneled facade glazing, roof ribs, and stronger LoD2/Wikimedia-informed landmark massing. It also keeps aligned landmark rail/reference-map numbering from shared `tour_order` data, DZI tile-pyramid release-readiness checks, expanded 26-landmark OSM/LoD2 placement QA, 68 cleanly attributed Wikimedia facade/context references, a refreshed top-down reference map, a public-data correctness crosscheck, geographically coherent landmark tour order, indexed landmark rail, focus progress bar, release-readiness guard, shareable landmark/view links, hash-based view restore, a tighter mobile toolbar, keyboard help, optional remote DZI hosting, fixed marker overlay listeners, visible previous/next landmark controls, disabled controls while the viewer is loading, restored focus after the reference-map modal closes, robust Escape handling for overlays, a landmark tour, keyboard next/previous/tour/home/link controls, clearer role-colored landmark pins, stronger selected-marker focus, relative-placement failures wired into QA status, denser building facade/window detail, stricter landmark-placement QA, cleaner current-day Wikimedia visual references, crisper deterministic pixel-art styling, richer LoD2/OSM/Wikimedia-derived visual detail, corrected landmark alignment against OSM/LoD2, a north-up default view, north/east/south/west rotation and mirror controls, an in-viewer top-down reference map with scale/north arrow, robust local launchers, deterministic packaging, and current quickstart docs. The AI tile style pass is still planned.
