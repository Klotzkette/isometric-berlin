...
describe("west Tiergarten extrapolation and the recessed Spree", () => {
  ...
  test("old displaced Bismarck is removed while Siegessäule retains its mosaic ink", async () => {
    const { createSiegessaeule } = await import("../src/IsometricCityWorld");
    const { AXIS_TO } = await import("../src/worldEnvelope");
    const column = createSiegessaeule();
    const bodies = column.getObjectByName("Siegessäule and Bismarck bodies") as Mesh;
    const position = bodies.geometry.getAttribute("position");
    const vertex = new Vector3();
    let oldSiteVertices = 0;
    for (let i = 0; i < position.count; i += 1) {
      vertex.fromBufferAttribute(position, i);
      if (Math.abs(vertex.x - (AXIS_TO[0] + 24)) < 12 &&
          Math.abs(vertex.z - (AXIS_TO[1] - 118)) < 12) oldSiteVertices += 1;
    }
    expect(oldSiteVertices).toBe(0);
    const ink = column.getObjectByName("Siegessäule and Bismarck ink lines") as LineSegments;
    expect(ink.geometry.getAttribute("position").count).toBeGreaterThan(0);
  });
