...
describe("lighter Minecraft tree cover", () => {
  test("removes about one sixth of each previous profile without relocating trees", () => {
    for (const profile of ["full", "mobile"] as const) {
      let previousCount = 0;
      let retainedCount = 0;
      for (const [x, z] of sourceTrees) {
        // Frozen v0.72.43 selection: verifies that thinning is a subset rather
        // than a reshuffle which would add different trees elsewhere.
        const oldHash = (Math.imul(x, 73_856_093) ^ Math.imul(z, 19_349_663)) >>> 0;
        const previouslyVisible = oldHash % 3 < (profile === "full" ? 2 : 1);
      ...
      expect(retainedCount / previousCount).toBeGreaterThan(0.8);
      expect(retainedCount / previousCount).toBeLessThan(0.86);
    }
  });
