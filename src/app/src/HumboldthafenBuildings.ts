...
export function planHumboldthafenBuildingDetails(prisms?: readonly HarbourPrism[], minecraft = false, mobileLike = false): Block[] {
  ...
  for (const p of parts) {
    const eins = HUMBOLDTHAFEN_EINS_IDS.has(p.id), base = p.y0_dm / 10, top = (p.y0_dm + p.h_dm) / 10;
    ...
    for (const ring of [p.ring, ...(p.holes ?? [])]) {
      for (let edge = 0; edge < ring.length; edge++) {
        ...
        const observedFloors = p.id === "3zV00024" || p.id === "ee9JgIcN" ? 7
          : p.id === "nOu64nI2" || p.id === "FxtigPVE" ? 2 : Math.round((top - base) / 4);
        ...
        for (let bay = 0; bay < bays; bay++) {
          ...
          for (let level = 0; level < floors; level++) {
            ...
            if (eins) {
              ...
              if (!minecraft && !mobileLike) emit((bay + .02) * pitch + finWidth * .8, y, finWidth * .28, floor - .13, .14, .43, STONE, "folded-fin-lip");
            } else {
          ...
          if (length > 10 && (eins ? nx < -.7 : nz > .65)) emit(bay * pitch + .3, base + floor / 2, .5, floor, .6, .3, FIN, "waterfront-colonnade");
        }
      }
    }
    if (minecraft) {
...
export function createHumboldthafenBuildingDetails(payload?: { buildings: readonly HarbourPrism[] }, options: { minecraft?: boolean; mobileLike?: boolean } = {}): Group {
  ...
  if (options.minecraft) {
    ...
    const flatRoles = new Set(["window", "commercial-glazing", "window-mullion", "stone-panel-joint"]);
    blocks.forEach(b => {
