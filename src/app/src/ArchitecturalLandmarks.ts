...
/**
 * Stable west-elevation articulation read from the owner's frontal reference.
...
function archedWindowFrameGeometry(
  width: number,
  height: number,
  frameM = 0.16,
): ShapeGeometry {
  ...
  const spring = height / 2 - radius;
  const outer = new Shape();
  outer.moveTo(-radius, bottom);
  outer.lineTo(radius, bottom);
  outer.lineTo(radius, spring);
  outer.absarc(0, spring, radius, 0, Math.PI, false);
  outer.lineTo(-radius, bottom);
  outer.closePath();

  const innerRadius = radius - frameM;
  const innerBottom = bottom + frameM;
  const innerSpring = spring;
  const opening = new Path();
  opening.moveTo(-innerRadius, innerBottom);
  opening.lineTo(-innerRadius, innerSpring);
  opening.absarc(0, innerSpring, innerRadius, Math.PI, 0, true);
...
function addReichstagWindowSets(
  ...
  for (const side of [-1, 1]) {
    ...
    for (let index = 0; index < shortCount; index += 1) {
      ...
      if (side > 0) {
        upper.push({
          position: [side * (signature.width_m / 2 + 0.14), 16.5, z],
          rotation: [0, Math.PI / 2, 0],
        });
      }
    }
  ...
  // The photographed west elevation has five regular bays between the
  // portico and each corner pavilion. Keeping them explicit avoids the old
  // four-bay approximation and leaves the central sculptural fields clear.
  for (const side of [-1, 1]) {
    for (
      let index = 0;
      index < REICHSTAG_WEST_FACADE_PROFILE.wingBayCountPerSide;
      index += 1
    ) {
      const z = side * (23.5 + index * 6.2);
      arched.push({
        position: [-(signature.width_m / 2 + 0.18), 9.1, z],
        rotation: [0, Math.PI / 2, 0],
      });
      upper.push({
        position: [-(signature.width_m / 2 + 0.2), 16.5, z],
        rotation: [0, Math.PI / 2, 0],
      });
    }
  }
  ...
  for (const xSide of [-1, 1]) {
    for (const zSide of [-1, 1]) {
      ...
      for (const offset of [-4.1, 0, 4.1]) {
        ...
        // The upper pavilions stand above the main roof and expose all four
        // faces. Drawing only the two outward faces made the near pavilion a
        // blank cube in oblique views.
        for (const faceSide of [-1, 1]) {
          const towerFaceZ =
            faceSide === zSide
              ? zSide * (signature.depth_m / 2 + 0.17)
              : towerZ + faceSide * (towerSize / 2 + 0.17);
          const towerFaceX =
            faceSide === xSide
              ? xSide * (signature.width_m / 2 + 0.17)
              : towerX + faceSide * (towerSize / 2 + 0.17);
          towerUpper.push(
            {
              position: [towerX + offset, 20.55, towerFaceZ],
            },
            {
              position: [towerFaceX, 20.55, towerZ + offset],
              rotation: [0, Math.PI / 2, 0],
            },
          );
        }
  ...
  const upperGeometry = new PlaneGeometry(2.15, 3.05);
  ...
  addInstancedGeometry(
    group,
    "Reichstag instanced tall arched-window stone reveals",
    archedWindowFrameGeometry(2.78, 6.52, 0.19),
    windowMetal,
    arched,
  );
  addInstancedGeometry(
  ...
  addInstancedGeometry(
    ...
    rectangularWindowFrameGeometry(2.55, 3.45, 0.17),
    windowMetal,
    upper,
  );
  addInstancedGeometry(
  ...
  addInstancedGeometry(
    ...
    archedWindowGeometry(2.15, 4.35),
    darkGlass,
    towerUpper,
  );
  addInstancedGeometry(
    ...
    archedWindowFrameGeometry(2.55, 4.75, 0.17),
    windowMetal,
    towerUpper,
  );
  addInstancedGeometry(
    group,
    "Reichstag instanced lower tower arched-window stone reveals",
    archedWindowFrameGeometry(2.7, 7.25, 0.18),
    windowMetal,
    towerArches,
  );
  addInstancedBoxes(
...
function addReichstagWestFacadeArticulation(
  ...
  const towerSize = REICHSTAG_TOWER_SIZE_M;
  const towerCentres = [-1, 1].map((side) =>
    reichstagTowerCentre(side, signature.depth_m),
  );

  for (const y of [4.35, 18.25, signature.body_height_m - 2.25]) {
    addBox(
      group,
      "Reichstag west continuous facade cornice",
      [0.44, y === 4.35 ? 0.58 : 0.42, signature.depth_m + 0.6],
      [westFaceX - 0.18, y, 0],
      reliefStone,
      0.58,
    );
  ...
  // The two western corner towers have a real upper arcade. A shallow stone
  // field gives the existing arched openings a facade to sit in without
  // changing or covering the measured LoD2 envelope beneath it.
  addInstancedBoxes(
    group,
    "Reichstag west corner-tower upper arcade fields",
    [0.18, 11.8, towerSize - 0.8],
    reliefStone,
    towerCentres.map((z) => ({
      position: [westFaceX + 0.05, signature.body_height_m - 5.9, z],
    })),
  );
  const westTowerX = reichstagTowerCentre(-1, signature.width_m);
  addInstancedBoxes(
    group,
    "Reichstag west tower lateral upper arcade fields",
    [towerSize - 0.8, 11.8, 0.18],
    reliefStone,
    [-1, 1].map((side) => ({
      position: [
        westTowerX,
        signature.body_height_m - 5.9,
        side * (signature.depth_m / 2 + 0.05),
      ],
    })),
  );

  const wingPilasters: InstanceTransform[] = [];
  const wingPilasterBases: InstanceTransform[] = [];
  const wingPilasterCapitals: InstanceTransform[] = [];
  for (const side of [-1, 1]) {
    for (let index = 0; index <= 5; index += 1) {
      const z = side * (20.4 + index * 6.2);
      const transform = {
        position: [westFaceX - 0.27, 11.1, z] as [number, number, number],
      };
      wingPilasters.push(transform);
      wingPilasterBases.push({
        position: [westFaceX - 0.34, 4.35, z],
      });
      wingPilasterCapitals.push({
        position: [westFaceX - 0.34, 17.85, z],
      });
    }
  }
  addInstancedBoxes(
  ...
  addInstancedBoxes(
    group,
    "Reichstag west wing pilaster bases",
    [0.62, 0.5, 1.05],
    reliefStone,
    wingPilasterBases,
  );
  addInstancedBoxes(
    group,
    "Reichstag west wing pilaster capitals",
    [0.68, 0.62, 1.22],
    reliefStone,
    wingPilasterCapitals,
  );

  const towerPilasters: InstanceTransform[] = [];
  const towerHorizontalOrders: InstanceTransform[] = [];
  for (const towerCentre of towerCentres) {
    for (const offset of [-7, -2.15, 2.15, 7]) {
      towerPilasters.push({
        position: [westFaceX - 0.3, 16.15, towerCentre + offset],
      });
    }
    for (const y of [17.85, 25.15]) {
      towerHorizontalOrders.push({
        position: [westFaceX - 0.3, y, towerCentre],
      });
    }
  }
  addInstancedBoxes(
  ...
  addInstancedBoxes(
    group,
    "Reichstag west corner-tower horizontal orders",
    [0.62, 0.58, towerSize + 1.5],
    reliefStone,
    towerHorizontalOrders,
  );

  const towerSidePilasters: InstanceTransform[] = [];
  const towerSideOrders: InstanceTransform[] = [];
  for (const side of [-1, 1]) {
    for (const offset of [-7, -2.15, 2.15, 7]) {
      towerSidePilasters.push({
        position: [
          westTowerX + offset,
          16.15,
          side * (signature.depth_m / 2 + 0.3),
        ],
      });
    }
    for (const y of [17.85, 25.15]) {
      towerSideOrders.push({
        position: [westTowerX, y, side * (signature.depth_m / 2 + 0.3)],
      });
    }
  }
  addInstancedBoxes(
  ...
  addInstancedBoxes(
    group,
    "Reichstag west corner-tower lateral horizontal orders",
    [towerSize + 1.5, 0.58, 0.62],
    reliefStone,
    towerSideOrders,
  );

  const towerQuoins: InstanceTransform[] = [];
  for (let level = 0; level < 9; level += 1) {
    const y = 5.0 + level * 2.58;
    for (const towerCentre of towerCentres) {
      for (const edge of [-1, 1]) {
        towerQuoins.push({
          position: [westFaceX - 0.42, y, towerCentre + edge * 7.2],
        });
      }
    }
    for (const side of [-1, 1]) {
  ...
  const towerBalusters: InstanceTransform[] = [];
  for (const side of [-1, 1]) {
    const towerCentre = reichstagTowerCentre(side, signature.depth_m);
    for (let index = 0; index < 6; index += 1) {
      const along = -6.2 + index * 2.48;
      towerBalusters.push(
        {
          position: [
            westFaceX - 0.15,
            signature.body_height_m + 3.35,
            towerCentre + along,
          ],
        },
        {
  ...
  const towerLedgeFigures: InstanceTransform[] = [];
  for (const side of [-1, 1]) {
    const towerCentre = reichstagTowerCentre(side, signature.depth_m);
    for (const edge of [-1, 1]) {
      towerLedgeFigures.push(
        {
          position: [westFaceX - 0.78, 19.4, towerCentre + edge * 7.35],
        },
        {
          position: [
            westTowerX + edge * 7.35,
            19.4,
            side * (signature.depth_m / 2 + 0.78),
          ],
        },
      );
    }
  ...
  addInstancedGeometry(
    ...
    new CapsuleGeometry(0.36, 1.45, 4, 8),
    stone,
    towerLedgeFigures,
  );
  addInstancedGeometry(
    group,
    "Reichstag west tower ledge allegorical figure heads",
    new SphereGeometry(0.31, 9, 7),
    reliefStone,
    towerLedgeFigures.map((transform) => ({
      position: [
        transform.position[0],
  ...
  const basementWindows: InstanceTransform[] = [];
  for (const side of [-1, 1]) {
    for (let index = 0; index < 8; index += 1) {
      basementWindows.push({
        position: [westFaceX - 0.22, 2.1, side * (25.8 + index * 5.25)],
        rotation: [0, Math.PI / 2, 0],
      });
    }
  }
  addInstancedGeometry(
    ...
    new PlaneGeometry(1.65, 1.35),
    basementGlass,
    basementWindows,
  );
  addInstancedGeometry(
    group,
    "Reichstag west rusticated-base window frames",
    rectangularWindowFrameGeometry(2.02, 1.68, 0.13),
    reliefStone,
    basementWindows,
  );

  const dentils: InstanceTransform[] = Array.from(
    { length: REICHSTAG_WEST_FACADE_PROFILE.corniceDentilCount },
    (_, index) => ({
      position: [
        westFaceX - 0.38,
  ...
  for (const z of panelCentres) {
    const panel = addBox(
      group,
      "Reichstag west portico sculptural relief panel",
      [0.34, 8.7, 5.1],
      [-signature.width_m / 2 - 2.72, 8.4, z],
      reliefStone,
      0.72,
    );
  ...
  addInstancedGeometry(
    ...
    new CapsuleGeometry(0.34, 1.35, 4, 8),
    reliefStone,
    roofFigures,
  );
  addInstancedGeometry(
    group,
    "Reichstag west roofline allegorical figure heads",
    new SphereGeometry(0.3, 10, 8),
    stone,
    roofFigures.map((transform) => ({
      position: [
        transform.position[0] - 0.05,
        transform.position[1] + 1.18,
        transform.position[2],
      ],
    })),
  );
  addInstancedBoxes(
    group,
    "Reichstag west roofline figure pedestals",
    [0.62, 0.42, 0.82],
    reliefStone,
    roofFigures.map((transform) => ({
      position: [
        transform.position[0] + 0.03,
        transform.position[1] - 1.04,
        transform.position[2],
      ],
    })),
  );
...
function addReichstagDocumentedOrders(
  ...
  addInstancedGeometry(
    ...
    new CylinderGeometry(0.42, 0.68, 2.15, 10),
    stone,
    pinnacles,
  );
  ...
  const FLUTES = REICHSTAG_WEST_FACADE_PROFILE.columnFluteCount;
  ...
  for (let column = 0; column < 6; column += 1) {
    ...
    for (let leaf = 0; leaf < 8; leaf += 1) {
      ...
      capitalLeaves.push({
        position: [
          westX + Math.cos(angle) * 1.28,
          18.13,
          cz + Math.sin(angle) * 1.28,
        ],
        rotation: [0, -angle, 0],
  ...
  const architraveDentils: InstanceTransform[] = Array.from(
    { length: REICHSTAG_WEST_FACADE_PROFILE.architraveDentilCount },
    (_, index) => ({
      position: [
        westX - 3.86,
        19.78,
        -18.5 +
          (index / (REICHSTAG_WEST_FACADE_PROFILE.architraveDentilCount - 1)) *
            37,
      ] as [number, number, number],
    }),
  );
  ...
  const rakingMouldings: VectorSegment[] = [];
  for (const lift of [0, 0.28, 0.56]) {
    rakingMouldings.push(
      [
        [westX - 3.92, 20.2 + lift, -19.5],
        [westX - 3.92, 26.5 + lift, 0],
      ],
      [
        [westX - 3.92, 26.5 + lift, 0],
        [westX - 3.92, 20.2 + lift, 19.5],
  ...
  addVectorSegments(
    group,
    "Reichstag west pediment triple raking moulding",
    rakingMouldings,
    EDGE_COLOR,
    0.86,
  );
  ...
  for (
    let index = 0;
    index < REICHSTAG_WEST_FACADE_PROFILE.tympanumFigureCount;
    index += 1
  ) {
    const z =
      -9.25 +
  ...
  for (const xSide of [-rx, rx]) {
    for (let index = 0; index <= 32; index += 1) {
      const z = -rz + (index / 32) * (rz * 2);
      const yTop = index % 2 === 0 ? 2.7 : 4.05;
      rustication.push([
        [xSide, 0.1, z],
        [xSide, yTop, z],
...
function createReichstagModel(signature: ReichstagModelSignature): Group {
  ...
  for (let index = 0; index < 6; index += 1) {
    ...
    addBox(
      group,
      `Reichstag west portico Corinthian abacus ${index + 1}`,
      [2.9, 0.3, 2.9],
      [westX, 18.58, -17.5 + index * 7],
      stoneRelief,
      0.58,
    );
  ...
  for (
    let index = 0;
    index < REICHSTAG_WEST_FACADE_PROFILE.centralGlassBayCount;
    index += 1
  ) {
    addBox(
      group,
  ...
  addInstancedBoxes(
    ...
    [0.16, 9.05, 0.18],
    modelMaterial(0x596466, { metalness: 0.34, roughness: 0.44 }),
    [-7.3, -2.5, 2.5, 7.3].map((z) => ({
      position: [westX + 0.42, 8.35, z],
    })),
  );
  addInstancedBoxes(
    group,
    "Reichstag west entrance bronze transoms",
    [0.16, 0.16, 14.8],
    modelMaterial(0x596466, { metalness: 0.34, roughness: 0.44 }),
    [5.55, 8.35, 11.15].map((y) => ({
      position: [westX + 0.41, y, 0],
    })),
  );
  addBox(
    group,
...
function createBrandenburgGateModel(
  ...
  const mainBodyWidth = BRANDENBURG_GATE_PHOTO_DETAIL_PROFILE.mainBodyWidthM;
  const columnBaseRadius =
    BRANDENBURG_GATE_PHOTO_DETAIL_PROFILE.columnBaseDiameterM / 2;
  const columnTopRadius = 0.72;
  const sidePassageWidth =
    BRANDENBURG_GATE_PHOTO_DETAIL_PROFILE.sidePassageWidthM;
  const centralPassageWidth =
    BRANDENBURG_GATE_PHOTO_DETAIL_PROFILE.centralPassageWidthM;
  const clearPassageWidths = [
    sidePassageWidth,
    sidePassageWidth,
    centralPassageWidth,
    sidePassageWidth,
  ...
  const columnAxisSpacings = clearPassageWidths.map(
    (width) =>
      width + BRANDENBURG_GATE_PHOTO_DETAIL_PROFILE.columnBaseDiameterM,
  );
  const columnAxisSpan = columnAxisSpacings.reduce(
    (sum, spacing) => sum + spacing,
    0,
  );
  const columnAxes = [-columnAxisSpan / 2];
  for (const spacing of columnAxisSpacings) {
    const previousAxis = columnAxes.at(-1) ?? -columnAxisSpan / 2;
    columnAxes.push(previousAxis + spacing);
  }
  ...
  for (let row = 0; row < rowX.length; row += 1) {
    ...
    for (let index = 0; index < columnAxes.length; index += 1) {
      const z = columnAxes[index];
      columnCenters.push([x, z]);
      const column = new Mesh(
        new CylinderGeometry(
          columnTopRadius,
          columnBaseRadius,
          signature.column_height_m,
          40,
        ),
        sandstone,
      );
      ...
      group.add(column);
      const base = new Mesh(
        new CylinderGeometry(1.02, 1.08, 0.42, 40),
        sandstone,
      );
      ...
      group.add(base);
      const capital = new Mesh(
        new CylinderGeometry(1.14, 0.76, 0.68, 40),
        sandstone,
      );
  ...
  for (const [columnX, columnZ] of columnCenters) {
    for (
      ...
      flutingSegments.push([
        [
          columnX + Math.cos(angle) * (columnBaseRadius - 0.035),
          0.72,
          columnZ + Math.sin(angle) * (columnBaseRadius - 0.035),
        ],
        [
          columnX + Math.cos(angle) * (columnTopRadius - 0.025),
          signature.column_height_m - 0.7,
          columnZ + Math.sin(angle) * (columnTopRadius - 0.025),
        ],
      ]);
    }
  }
    ...
    for (
      ...
      for (let segment = 0; segment < fluteSegmentCount; segment += 1) {
        ...
        const radius =
          columnBaseRadius -
          0.035 +
          (columnTopRadius - columnBaseRadius + 0.01) * progress;
        fluteGrooves.push({
          position: [
  ...
  addInstancedGeometry(
    ...
    new CylinderGeometry(1.0, 1.07, 0.18, 40),
    sandstone,
  ...
  addInstancedGeometry(
    ...
    new CylinderGeometry(0.77, 0.77, 0.16, 40),
    sandstoneShadow,
    columnCenters.map(([x, z]) => ({
  ...
  addInstancedGeometry(
    ...
    new CylinderGeometry(0.79, 0.79, 0.055, 40),
    sandstoneShadow,
  ...
  const passageDividers = columnAxes.slice(1, -1);
  ...
  for (const dividerZ of passageDividers) {
    ...
    for (const passageSide of [-1, 1]) {
      medallions.push({
        position: [0, 8.2, dividerZ + passageSide * 0.72],
        rotation: [0, passageSide < 0 ? Math.PI : 0, 0],
      });
      reliefPanels.push({
        position: [0, 3.75, dividerZ + passageSide * 0.725],
      });
    }
  }
  addInstancedGeometry(
  ...
  for (const z of [-1, 1]) {
    ...
    for (const x of [-1, 1]) {
      addBox(
        ...
        [0.08, 6.15, pavilionWidth - 2.7],
        [x * 4.92, 3.72, pavilionZ],
        recess,
      );
    }
    ...
    addInstancedGeometry(
      ...
      new CylinderGeometry(0.28, 0.34, 6.6, 24),
      sandstone,
      porticoColumns,
    );
    addInstancedGeometry(
      photoDetails,
      "Brandenburg Gate pavilion portico bases",
      new CylinderGeometry(0.42, 0.45, 0.2, 24),
      sandstone,
      porticoColumns.map((column) => ({
        position: [column.position[0], 0.1, column.position[2]],
      })),
    );
    addInstancedGeometry(
      photoDetails,
      "Brandenburg Gate pavilion portico capitals",
      new CylinderGeometry(0.44, 0.3, 0.28, 24),
      sandstone,
      porticoColumns.map((column) => ({
        position: [column.position[0], 6.72, column.position[2]],
      })),
    );
    addBox(
      group,
      "Brandenburg Gate pavilion cornice",
      [signature.depth_m, 0.62, pavilionWidth],
      [0, 7.35, pavilionZ],
      sandstone,
      0.78,
    );
    const roofHalfDepth = signature.depth_m / 2 - 0.16;
    const roofHalfWidth = pavilionWidth / 2 - 0.2;
    const roofEaveY = 9.48;
    const roofRidgeY = pavilionHeight;
    const roofZMin = pavilionZ - roofHalfWidth;
    const roofZMax = pavilionZ + roofHalfWidth;
    ...
    const roof = new Mesh(roofGeometry, patinatedCopper);
    roof.name = "Brandenburg Gate pavilion patinated gable roof";
    photoDetails.add(roof);
    addEdges(photoDetails, roof, 0.58);
    const courseHeight = 1.3;
    for (const xSide of [-1, 1]) {
      const faceX = xSide * (signature.depth_m / 2 - 0.01);
      for (
        let course = 1;
        course * courseHeight < pavilionBodyHeight;
        course += 1
      ) {
      ...
      for (
        ...
        for (
          ...
          pavilionMasonry.push([
            ...
            [
              faceX,
              Math.min(pavilionBodyHeight, yStart + courseHeight),
              jointZ,
            ],
          ]);
        }
  ...
  addBox(
    ...
    [signature.depth_m, 1.42, mainBodyWidth],
    [0, 14.48, 0],
    sandstone,
    0.9,
  );
  ...
  addBox(
    ...
    [signature.depth_m, 0.78, mainBodyWidth - 0.35],
    [0, 16.73, 0],
    sandstone,
    0.8,
  );
  addBox(
    ...
    [signature.depth_m - 0.55, 0.72, 23.8],
    [0, 19.26, 0],
    sandstoneShadow,
    0.82,
  );
  addBox(
    group,
    "Brandenburg Gate central Quadriga plinth",
    [signature.depth_m - 0.72, 0.68, 14.8],
    [0, 19.96, 0],
    sandstone,
    0.88,
  );
  ...
  for (const [name, y, depth, width] of [
    ["lower cornice", 13.72, 11, mainBodyWidth + 0.35],
    ["frieze crown", 17.28, 11, mainBodyWidth + 0.24],
    ["attic base crown", 18.94, 10.72, mainBodyWidth - 0.02],
    ["Quadriga plinth crown", 20.28, 10.42, 15.22],
  ] as const) {
    addBox(
      photoDetails,
  ...
  for (const xSide of [-1, 1]) {
    ...
    for (const y of [13.72, 14.48, 15.18, 15.76, 16.34, 17.28]) {
      entablatureProfiles.push([
        [faceX, y, -mainBodyWidth / 2],
        [faceX, y, mainBodyWidth / 2],
      ]);
    }
    for (const z of columnAxes) {
      entablatureProfiles.push([
        [faceX, 13.35, z],
        [faceX, 17.28, z],
      ]);
  ...
  quadriga.position.set(0, signature.gate_height_m, 0);
  group.add(quadriga);
