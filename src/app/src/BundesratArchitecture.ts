...
function make(payload: { buildings: readonly PrismBuilding[] } | undefined, options: BundesratOptions, mc: boolean): Group {
  const parts = (payload?.buildings ?? BUNDESRAT_SOURCE_PRISMS as unknown as PrismBuilding[]).filter(p => BUNDESRAT_IDS.has(p.id)), walls = bundesratWalls({ buildings: parts }), p: Plan = { blocks: [], curves: createBuilder(), mobile: mc && !!options.mobileLike, mc, windows: 0, push: mc ? sourcePush(walls, options.voxels) : new Map() };
