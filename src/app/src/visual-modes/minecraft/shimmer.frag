vec3 premiumShimmer(vec3 color, vec2 uv, float time) {
  ...
  float sparkle = pow(max(0.0, wave), 20.0);
  float twinkle = pow(max(0.0, sin((uv.x + uv.y) * 154.0 + time * 0.28)), 26.0);
  color += vec3(0.05, 0.075, 0.08) * sparkle * waterMask;
  color += vec3(0.075, 0.08, 0.07) * twinkle * glassMask;
  return color;
}
