...
async function build(input: ProgressiveWorldWorkerInput): Promise<void> {
  ...
  let batchCount = 0;
  let pretriangulated = true;
  const heavyPromise = loadHeavyPlates(input).catch(() => {
  ...
  // avoiding the one-shot tessellator's multi-GiB transient. It also keeps the
  // package 4.70 MiB smaller than committing a second plate.
  ...
  // Geometry is already merged by material inside each bounded batch. The
  // 5,000-building cap leaves six retained groups for today's production
  // payload instead of forty-two, while keeping each GPU upload progressive.
