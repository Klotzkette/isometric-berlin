...
**Minecraft is a true voxel world** (v0.6.0): switching in lazily loads
...
The block world fully replaces the photogrammetry surfaces, hero crops and
recognition layers while active — the city is cubes, nothing else — and is
rendered as four instanced meshes with deterministic per-block shade jitter
from the 28-colour palette, so it stays a handful of draw calls. Until the
payload arrives (or if it fails to load) the toon-material presentation is
the fallback. Leaving Minecraft restores the drawn photogrammetry scene
losslessly. A
